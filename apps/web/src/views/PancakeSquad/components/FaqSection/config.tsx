...
const config = ({ t }: FAQsType) => [
  {
    title: t('What is DynastySwap building?'),
    description: [
  ...
  },
  {
    ...
    description: [
      <Text as="p" color="textSubtle" fontSize="16px">
        {t('Yes, DynastySwap has multiple audits including an Audit, Bug Bounty and KYC by CertiK: ',)}
        <Link
          display="inline-flex"
          color="text"
          title="CertiK Audit"
  ...
  {
    title: t('How does DynastySwap combine AI with Blockchains?'),
    description: [
  ...
  {
    title: t('What is the DynastySwap AI DEX aggregator?'),
    description: [
      t('It\'s a sophisticated system that scans the entire blockchain to find the optimal way to execute your swap. ' +
        'It allows you to trade any token at the best rate, not only taking into account the DynastySwap liquidity, but also the liquidity of other DEXes. ' +
  ...
  {
    title: t('How does DynastySwap extract arbitrage for me?'),
    description: [
      t('On every trade, our AI DEX aggregator not only finds you the best way to execute that trade, but even scans for Arbitrage opportunities. ' +
        ...
        'As most Arbitrage is generated from inefficient trades, DynastySwap users not only don\'t loose value to Arbitrage, they even directly profit from users that still use less sophisticated DEXes.'),
    ],
  },
  {
    title: t('How to list a token on DynastySwap?'),
    description: [
